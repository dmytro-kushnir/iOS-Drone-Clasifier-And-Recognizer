...
utils.computeBearingIn360 = function (dx, dy) {
  ...
  if (angle > 0) {
    if (dy > 0) return angle;else return 180 + angle;
  } else {
    if (dx > 0) return 180 + angle;else return 360 + angle;
  }
};
  ...
  exports.ItemTracked = function (properties, frameNb, unMatchedFramesTolerance, fastDelete) {
    ...
    itemTracked.updateVelocityVector = function () {
      ...
      if (this.itemHistory.length <= exports.ITEM_HISTORY_MAX_LENGTH) {
        var start = this.itemHistory[0];
        var end = this.itemHistory[this.itemHistory.length - 1];
        return computeVelocityVector(start, end, this.itemHistory.length);
      } else {
        var _start = this.itemHistory[this.itemHistory.length - exports.ITEM_HISTORY_MAX_LENGTH];
        var _end = this.itemHistory[this.itemHistory.length - 1];
        return computeVelocityVector(_start, _end, exports.ITEM_HISTORY_MAX_LENGTH);
      }
    };
